...
export class RedisSubPub {
  ...
  constructor(
    ...
    private channelPrefix: string = 'image-search-channel#',
  ) {
    this.init()
  }
  ...
  public async publish(event: string, data: any) {
    ...
    if (event !== 'log')
      Logger.debug(`publish event：${channel} <- ${_data}`, RedisSubPub.name)
  ...
  public async subscribe(event: string, callback: (data: any) => void) {
    ...
    const cb = (channel, message) => {
      if (channel === myChannel) {
        if (event !== 'log')
          Logger.debug(`receive events：${channel} -> ${message}`, RedisSubPub.name)
